...
@Service("jiabiStatistics")
public class JiabiStatistics {
    ...
    public void run()
    {
    	...
    	for(int i=0;i<=count/onepage;i++){
    		...
    		for(JiabiBaseInfo jbi:readPage.getContent()){
    			...
    			//if(jiabiIdSet.contains(jbi.getFmid())) continue;
    			//jiabiIdSet.add(jbi.getFmid());
    	...
    	MapHelper.write(edges, "C:\\Users\\Daisy\\Desktop\\map.txt");
    }
    ...
    private void findCase(CaseBaseInfo cbi){
    	...
    	for(SuspectBaseInfo sbi:cbi.getSuspects()){
    		//if(suspectIdSet.contains(sbi.getId()))continue;
    		//jiabiIdSet.add(sbi.getId());
    ...
    private void findJiabi(JiabiBaseInfo jbi){
    	String start="J"+jbi.getFmid();
    	for(CaseBaseInfo cbi:jbi.getCaseInfos()){
    		//if(caseIdSet.contains(cbi.getId()))continue;
    		//caseIdSet.add(cbi.getId());
    		String end="C"+cbi.getId();
    		if(edges.containsKey(start)){
    			if(edges.get(start).contains(end)) continue;
    			else{
